...
- **Training data:** [train/data.py](../src/parallm/train/data.py) — mixtures are **JSON in [configs/data](../configs/data)**, selected by `--data-preset <name-or-path>`; adding a dataset never touches the code. A source declares `format: text | template | chat`, and `chat` runs `messages` through the tokenizer's chat template, which is what lets an SFT corpus be trained on at all. Default is `cascade2` — five `nvidia/Nemotron-Cascade-2-SFT-Data` subsets (math .30 / swe .20 / chat .20 / science .15 / instruction_following .15). ⚠⚠ **`weight` is a share of TOKENS, not the interleave probability.** `interleave_datasets` draws per *document*, and this mixture spans **922 to 48,340 tok/doc (52x)**, so the declared weights are divided by a measured mean length (`token_weights`); read as document probabilities, 30% of math's tokens is 5.2% of its documents. The mean length is PROBED from the first 64 docs of each source at startup (nothing is declared in the JSON — a declared length would only duplicate the measurement), and the realized split is logged (`[data] … % of tokens = … % of documents @ … tok/doc`) — read it, because nominal and realized came apart badly once (a nominal 34/33/33 trained as 66/32/2). ✅ **Trained on the RAW `<think>` traces: stripping them was A/B'd on this exact mixture and LOST** (tail mean 0.6322 vs **0.6409**, raw ahead at 6 of 8 evals with a tighter range; stripping led on math alone, 0.4288 vs 0.4075). There is deliberately no strip/scrub code. Rails: [tests/test_data_mixture.py](../tests/test_data_mixture.py).
- **Forward:** [model/pt_model.py](../src/parallm/model/pt_model.py) `PTWrappedModel` (lockstep window iteration + `SyncBoundary`). **`sync_phase` does not pick a WALK, it picks two SETS** — `sync_sets()` maps the name to `(post-attn sync layers, post-MLP sync layers)` and ONE loop (`_run_stack`, plus its batched mirror) serves all three: `post-attn` = `(boundaries, {last})` = L+1 all-reduces, `post-mlp` = `({}, boundaries ∪ {last})` = L, `exact` = both at every layer = 2L. `last` is always in the post-MLP set — the head needs a synced state. The trainer's teacher-forced block loop and `capture_sets` read the same method, so the schedule cannot drift between the walk that deploys and the loop that supervises. ⚠ **`post-mlp` used to be a second, hand-mirrored walk that never called `fuse`** and therefore had to REFUSE `fuse_size/exec_groups/fuse_ranks > 1` — i.e. it could not run on any real qwen3-32B/N=64 config (`exec_groups=8` at F=1) and had no probe or target contract; expressing it as two sets gets it the merged/batched path for free. Rails: `tests/test_pt_k2_local_sync.py` (post-mlp ≡ a longhand whole-layer reference loop; `sync_sets` per phase), `tests/test_track_fusion_equivalence.py` (batched post-mlp ≡ looped), `tests/test_train_distill.py` (post-mlp supervises every boundary, and the N=1 zero-loss identity).
...
- **Eval:** [eval/fidelity.py](../src/parallm/eval/fidelity.py) (KL/ppl), [eval/downstream.py](../src/parallm/eval/downstream.py) + [eval/lm_eval_adapter.py](../src/parallm/eval/lm_eval_adapter.py). **Judge recovery by downstream retention**, not KL/ppl — the proxy hid a real failure once (KL ~85% while hard-reasoning was ~22–33%). The macro is the 5-task set in `downstream.DEFAULT_TASKS` — **arc_easy / arc_challenge / mmlu_math_mc / mmlu_cs_mc / codemmlu_fim** (reasoning / math / knowledge / code), one constant shared by the trainer's in-loop eval and `scripts/eval_lm_harness.py` so the two cannot drift. `mmlu_math_mc` and `mmlu_cs_mc` are the SAME cais/mmlu rendering over two taxonomy subcategories, so a difference between them isolates the SUBJECT — the control that arc/code cannot provide, since their prompts, shot counts and option depths all differ. ⚠ **it supersedes the 5-task (hellaswag / winogrande / piqa) average that every `macro=` in `logs/` before 2026-08-11 reports — the two are not comparable, so re-baseline rather than compare across the change.** The last two tasks are local YAMLs in [configs/eval_tasks](../configs/eval_tasks), registered via lm-eval's `include_path`: adding a benchmark over any hub dataset is a YAML, not a code change. ⚠ **the math slot changed on 2026-08-21**: it was `mmlu_pro_math_mc` (TIGER-Lab/MMLU-Pro filtered to `category == "math"`, **up to 10 options, ~0.10 chance floor**) and is now `mmlu_math_mc` (cais/mmlu filtered to the original taxonomy's math subcategory — 1064 docs over abstract_algebra / college_mathematics / elementary_mathematics / high_school_mathematics / high_school_statistics, **4 options, 0.25 chance floor**), so **every `macro=` and math number in `logs/` before that date is on the old task and is not comparable**. `mmlu_cs_mc` (412 docs, 4 CS subjects) joined the macro on 2026-08-22, and `mmlu_pro_math_mc` left it the same day. All three YAMLs ship; name any of them in `--tasks` / `--eval-tasks` for a side-by-side. Neither is lm-eval's built-in `mmlu_pro_math` / `mmlu`, which are `generate_until` or group tasks; ours are lettered MC, the cheapest loglikelihood shape, because `PTLM` re-runs the whole track-sharded forward per request. A task that fails to load raises `MissingTasks` rather than being averaged away: dropping the math task, reliably the lowest scorer, *raises* the macro, so a hub outage would read as a win and could promote a worse checkpoint.
