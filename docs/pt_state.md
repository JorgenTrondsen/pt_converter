...
- **Eval:** [eval/fidelity.py](../src/parallm/eval/fidelity.py) (KL/ppl), [eval/downstream.py](../src/parallm/eval/downstream.py) + [eval/lm_eval_adapter.py](../src/parallm/eval/lm_eval_adapter.py). **Judge recovery by downstream retention**, not KL/ppl — the proxy hid a real failure once (KL ~85% while hard-reasoning was ~22–33%). The macro is the 4-task set in `downstream.DEFAULT_TASKS` — **arc_easy / arc_challenge / mmlu_pro_math_mc / codemmlu_fim** (reasoning / math / code), one constant shared by the trainer's in-loop eval and `scripts/eval_lm_harness.py` so the two cannot drift. ⚠ **it supersedes the 5-task (hellaswag / winogrande / piqa) average that every `macro=` in `logs/` before 2026-08-11 reports — the two are not comparable, so re-baseline rather than compare across the change.** The last two tasks are local YAMLs in [configs/eval_tasks](../configs/eval_tasks), registered via lm-eval's `include_path`: adding a benchmark over any hub dataset is a YAML, not a code change. `mmlu_pro_math_mc` is NOT lm-eval's built-in `mmlu_pro_math`, which is `generate_until` (CoT + regex) and unusable through `PTLM`'s loglikelihood-only adapter — it is the same MMLU-Pro data filtered to math and rendered as lettered MC, with **up to 10 options, so read it against a ~0.10 chance floor, not 0.25**. A task that fails to load raises `MissingTasks` rather than being averaged away: dropping `mmlu_pro_math_mc`, reliably the lowest scorer, *raises* the macro ~0.07, so a hub outage would read as a win and could promote a worse checkpoint.
